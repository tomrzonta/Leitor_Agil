...
class LeitorRapidoApp:
    def __init__(self, master):
        ...
        # Aplica as configurações iniciais (carregadas ou padrão)
        ...
        if caminho_arquivo:
            ...
            if self.texto_completo_paragrafos:
                ...
                for paragraph in self.texto_completo_paragrafos:
                    # --- CORREÇÃO AQUI: Garante que a capitalização original seja mantida ---
                    # E que a pontuação esteja anexada.
                    # Não aplica .lower() no parágrafo antes do re.findall
    ...
    def atualizar_exibicao_palavra(self):
        if self.indice_palavra_atual < len(self.palavras):
            ...
            ppm_alvo = self.velocidade_scale.get() # Velocidade alvo do usuário
            
            # Se a velocidade temporária é menor que a alvo, aumente
            if self.velocidade_leitura_atual_temp < ppm_alvo:
                self.velocidade_leitura_atual_temp += self.passo_aceleracao
                # Garante que não ultrapasse a velocidade alvo
                if self.velocidade_leitura_atual_temp > ppm_alvo:
                    self.velocidade_leitura_atual_temp = ppm_alvo
            
            # Use a velocidade temporária para o cálculo do tempo
    ...
    def iniciar_pausar_leitura(self):
        ...
        if self.esta_lendo:
            ...
            # Ao iniciar/continuar, resetamos a velocidade temporária para a inicial de aceleração
            self.velocidade_leitura_atual_temp = self.velocidade_inicial_aceleracao
            if self.velocidade_leitura_atual_temp > self.velocidade_scale.get(): # Não comece mais rápido que o alvo
    ...
    def resetar_leitura(self):
        ...
        # Reseta a velocidade temporária para o valor alvo quando resetado
        self.velocidade_leitura_atual_temp = self.velocidade_scale.get() 
    ...
    def carregar_configuracoes(self):
        ...
        if os.path.exists(self.settings_file):
            try:
                ...
                # Estas linhas só devem ser executadas DEPOIS que os widgets existem.
                # A chamada para aplicar_estilo_fonte() no __init__ já cuida disso.
                # Se carregamos configurações depois do __init__, podemos chamar.
