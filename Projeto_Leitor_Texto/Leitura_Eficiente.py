...
# Você DEVE mudar este caminho para o local onde o Tesseract OCR está instalado no seu computador.
...
class LeitorRapidoApp:
    ...
    def carregar_arquivo(self, caminho_predefinido=None, indice_predefinido=0):
        ...
        if caminho_arquivo:
            ...
            # Retornar texto como uma única string bruta para limpeza global
            ...
            if texto_bruto_extraido:
                ...
                # 1. Substitui hífens no final de uma palavra e possíveis quebras por NADA (junta palavras hifenizadas).
                #    Ex: "esfarra-\npados" -> "esfarrapados"
                processed_text = re.sub(r'(\w+)-\s*?\n*\s*?(\w+)', r'\1\2', texto_bruto_extraido)
                
                # 2. Remove hífens que estão entre letras e não são hífens gramaticais (ex: "livr-o" -> "livro")
                #    Isso lida com o caso específico do "livr"
                processed_text = re.sub(r'([a-zA-Záéíóúçãõâêôàüñ])-(?=[a-zA-Záéíóúçãõâêôàüñ])', r'\1', processed_text)
                
                # 3. Substitui TODAS as quebras de linha por um único espaço.
                processed_text = processed_text.replace('\n', ' ')
                
                # 4. Remove referências de rodapé como "" ou "[X]".
    ...
    def atualizar_exibicao_palavra_sem_avancar(self):
        ...
        if self.indice_palavra_atual < len(self.palavras):
            ...
            if self.indice_palavra_atual < 0:
                # CORREÇÃO AQUI: Remove o parêntese extra
                self.palavra_label.config(text="Fim da leitura!") 
                self.indice_palavra_atual = 0
            else:
                self.palavra_label.config(text=self.palavras[self.indice_palavra_atual])
